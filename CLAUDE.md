...
- **Deploy**: free-tier stack, chosen because Railway's free plan grants only $1/month of usage credit (not enough for an always-on service + DB). Backend on **Render** (`server/render.yaml`, free web service — sleeps after 15 min idle, cold start on wake); DB on **Supabase Postgres** (free, pooled connection string on port 6543 as `DATABASE_URL`); frontend on **Vercel or Netlify** (`vercel.json` / `netlify.toml` at the repo root, both just SPA-rewrite `dist/` to `index.html` — pick one host, not both). `server/railway.json` is kept as a documented fallback, not the primary path.
